...
```{r}
loocv_loss_values <- round(rbind(linear_model_loocv_loss_values, ar1_model_loocv_loss_values), 3)
rownames(loocv_loss_values) <- c("Linear LOOCV", "AR1 LOOCV")
loocv_loss_values

lgocv_prior_loss_values <- round(rbind(linear_model_lgocv_prior_loss_values, ar1_model_lgocv_prior_loss_values), 3)
rownames(lgocv_prior_loss_values) <- c("Linear LGOCV Prior", "AR1 LGOCV Prior")
lgocv_prior_loss_values

lgocv_posterior_loss_values <- round(rbind(linear_model_lgocv_posterior_loss_values, ar1_model_lgocv_posterior_loss_values), 3)
...
linear_model_loss_values <- data.frame(MAPE = numeric(n_shift),
                                       RMSPE = numeric(n_shift))

ar1_model_loss_values <- data.frame(MAPE = numeric(n_shift),
                                    RMSPE = numeric(n_shift))

ar1_fixed_model_loss_values <- data.frame(MAPE = numeric(n_shift),
                                          RMSPE = numeric(n_shift))
