...
class KMeans:
    ...
    def __init__(self, k, iterations = None):
    ...
    def fit(self, X):
        ...
        """
        X = pd.DataFrame(X)
        self.centroids = pd.DataFrame(np.random.uniform(np.amin(X, axis = 0), np.amax(X, axis = 0), size = (self.k, X.shape[1])), columns = ["x0","x1"])
    ...
    def get_centroids(self, X):
        ...
        for j in range(self.k):
            ...
            if (len(x) != 0):
                new_pos[j] = [float(x["x0"].mean()), float(x["x1"].mean())]
            
        return pd.DataFrame(new_pos, columns = ["x0","x1"])
